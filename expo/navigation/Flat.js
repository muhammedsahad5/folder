import React,{useState}  from 'react';
import { SafeAreaView, View, FlatList, StyleSheet, Text, StatusBar, Alert, Image, TouchableOpacity } from 'react-native';
import { List, Avatar } from 'react-native-paper';
import Icon2 from 'react-native-vector-icons/Entypo';


const DATA = [
    {
        id: '1',
        number: '7523695412',
        name: "sahad",
        image: "https://www.shutterstock.com/image-photo/head-shot-portrait-close-smiling-260nw-1714666150.jpg"
    },
    {
        id: '2',
        number: '7812582615',
        image: "https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcTW0x5TosFMcZJYcMDgDusV-A6TvBBOo1Ju2w&usqp=CAU",
        name: "mirshan",
    },
    {
        id: '3',
        number: '8854589656',
        image: "https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcRZIwylh1lB6GKXeubHbTPp2lnIYvZittUXLA&usqp=CAU",
        name: "ashikh",

    },
    {
        id: '3',
        number: '7856985212',
        image: "data:image/jpeg;base64,",
        name: "Basil",

    },
];

const Item = ({ title }) => (
    <View style={styles.item}>
        <Text style={styles.title}>{title}</Text>
    </View>
);

const App = (props) => {
    const{route}=props
    const renderItem = ({ item }) => (
        <Item title={item.title} />
    );

    return (
        <SafeAreaView style={styles.container}>
            <FlatList
                data={DATA}
                renderItem={({ item }) => {
                    // Alert.alert("jj",JSON.stringify(item))
                    return (
                        <List.Item
                            style={{ backgroundColor: "#fff", margin: 2 }}
                            title={item.name}
                            description={item.number}
                            left={props =>
                               
                                <Avatar.Image size={50} source={{ uri: item.image }} />

                            }
                            right={props => {
                                return (
                                    <TouchableOpacity hitSlop={{ top: 20, bottom: 20, left: 50, right: 50 }} onPress={() => {
                                        alert("hiiiiiiiii")
                                    }}>
                                        <Icon2 style={{top:15}} size={20} color="green" name="phone"
                                        />
                                    </TouchableOpacity>
                                )
                            }

                            }
                            onPress={() => {
                                props.navigation.navigate("Sahad",{DATA:item})
                            }}
                        />

                    )
                }}
                keyExtractor={item => item.id}
            />
        </SafeAreaView>
    );
}

const styles = StyleSheet.create({
    container: {
        flex: 1,
        marginTop: StatusBar.currentHeight || 0,
    },
    item: {
        backgroundColor: '#f9c2ff',
        padding: 20,
        marginVertical: 8,
        marginHorizontal: 16,
    },
    title: {
        fontSize: 32,
    },
});

export default App;